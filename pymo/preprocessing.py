...
class MocapParameterizer(BaseEstimator, TransformerMixin):
    ...
    #     Parameters:
    #         X (list): List of track objects containing skeletons and rotation data.
    #
    #     Returns:
    #         list: A list of new tracks with joint positions.
    #     """
    #
    #     Q = []
    #     for track in X:
    ...
    #
    #         # Identify rotation and position columns
    #         rot_cols = [col for col in euler_df.columns if 'rotation' in col]
    #         pos_cols = [col for col in euler_df.columns if 'position' in col]
    #
    ...
    def _to_pos(self, X):
        """
        Converts joint rotations in Euler angles to joint positions.
        ...
        for track in X:
            ...
            # Initialize DataFrame for joint positions
            pos_df = pd.DataFrame(index=euler_df.index)
            ...
            for joint in track.traverse():
                ...
                # Extract relevant rotation and position columns for this joint
                joint_rot_cols = [col for col in rot_cols if joint in col]
                joint_pos_cols = [col for col in pos_cols if joint in col]

                # Prepare rotation and position data
                euler_values = (
                    euler_df[joint_rot_cols].reindex(columns=[f'{joint}_{axis}rotation' for axis in rot_order], fill_value=0)
                    .to_numpy()
                )
                pos_values = (
                    euler_df[joint_pos_cols]
                    .reindex(columns=[f'{joint}_{axis}position' for axis in 'XYZ'], fill_value=0)
                    .to_numpy()
                )

                # Convert Euler angles to rotation matrices
                rot_mats = R.from_euler(rot_order, euler_values, degrees=True).inv()
                tree_data[joint] = [[], []]  # Initialize storage for rotation matrices and positions

                if joint == track.root_name:
                    # Root joint
                    tree_data[joint][0] = rot_mats
                    tree_data[joint][1] = pos_values
                else:
                    # Calculate rotations and positions for non-root joints
                    tree_data[joint][0] = rot_mats * tree_data[parent][0]

                    # Apply offsets and transform positions
                    offsets = np.array(track.skeleton[joint]['offsets'])
                    global_positions = pos_values + offsets
                    local_positions = tree_data[parent][0].inv().apply(global_positions)
                    tree_data[joint][1] = tree_data[parent][1] + local_positions

            # # Create position DataFrame
            # for joint, (_, joint_positions) in tree_data.items():
            #     pos_df[f'{joint}_Xposition'] = joint_positions[:, 0]
            #     pos_df[f'{joint}_Yposition'] = joint_positions[:, 1]
            #     pos_df[f'{joint}_Zposition'] = joint_positions[:, 2]
            # Collect new columns for positions in a dictionary
            position_columns = {}

            for joint, (_, joint_positions) in tree_data.items():
                position_columns[f'{joint}_Xposition'] = joint_positions[:, 0]
                position_columns[f'{joint}_Yposition'] = joint_positions[:, 1]
                position_columns[f'{joint}_Zposition'] = joint_positions[:, 2]

            # Convert the dictionary to a DataFrame
            position_df = pd.DataFrame(position_columns, index=pos_df.index)

            # Add all new columns to pos_df at once
            pos_df = pd.concat([pos_df, position_df], axis=1)

            # Create a new track with the updated positions
            new_track = track.clone()
            new_track.values = pos_df
            Q.append(new_track)
    ...
    def _to_expmap(self, X):
        ...
        for track in X:
            ...
            rots = [c for c in euler_df.columns if ('rotation' in c and 'Site' not in c)]

            # List the joints that are not end sites, i.e., have channels
            joints = (joint for joint in track.skeleton if 'Site' not in joint)
